# [018-1] Murphy's Pond CH4 Monitoring

## Overview
- **Client/customer:** Murray State University / Dr. Bassil El Masri
- **Dollar value:** $86,862.94 total funding to Black Swift Technologies
- **Timeline:** Funding expires 2026-12-31. Flights #1 & #2 completed (Nov & Dec 2025). Flight #3 confirmed (task closed 2026-05-13); due 2026-06-30. S2 rental due 2026-06-01.
- **Status:** **Active.** Per Jack Elston (2026-04-20), Murphy's Pond competes with INSTAAR x2 and Ottawa demo for scheduling priority; execution order is "whatever actually happens first" among competing projects. Flight #3 confirmation resolved (2026-05-13), but execution timing remains subject to competing project demands.
- **Team members involved:** Jack Elston (lead technical/flight coordination), Maciej Stachura (payload configuration), Nate Straus (software), Meredith O'hara Needham (communications), Beck Cotter (owner/project lead)
- **Risk signals:** 
  - **Flight #3 execution uncertain** — confirmed dates resolved (task closed 2026-05-13) but subject to competing INSTAAR x2 and Ottawa demo scheduling; due 2026-06-30 but may slip depending on other projects
  - S2 rental order window (2026-06-01) imminent; training/travel required but **travel not yet scheduled**
  - **Flight #4 funding confirmation** (Beck Cotter to confirm with Bassil by 2026-05-20) — $31k available for potential additional flight; confirmation **overdue or imminent**
  - Unresolved technical issues: FLIR triggering failures, methane payload data rate investigation, camera overlap (FLIR + A5100) target 80% not yet configured
  - Funding expires 2026-12-31; all work must complete by year-end

## Key Deliverables & Milestones
- **Flight #1:** Completed November 11, 2025
- **Flight #2:** Completed December 11, 2025
- **Flight #3:** Confirmed dates (task closed 2026-05-13); Due: 2026-06-30
- **Flight #4 Funding Confirmation:** Due 2026-05-20 (Beck Cotter to confirm with Bassil) — **STATUS: OVERDUE/IMMINENT**
- **S2 Aircraft Rental (2 units, S0 VTOL):** Due 2026-06-01; training/travel required; travel not yet scheduled
- **Optional Flight #4:** $31k available funding (Dr. Masri offer from July 2025); confirmation pending

## Task Summary
- **Total tasks:** 1 open
- **Tasks by assignee:**
  - **Beck Cotter:** 1 open task
    - Confirm Bassil has funding for Flight #4 (Due: 2026-05-20) — **OVERDUE/IMMINENT**
- **Notable patterns:** Project now in execution phase; critical path centers on Flight #4 funding confirmation (due 2026-05-20, appears overdue) and Flight #3 execution (2026-06-30); technical issues remain outstanding

## Recent Activity
- **2026-05-13:** Flight #3 confirmation task completed (Jack Elston); dates now confirmed with Dr. Masri
- **2026-04-20 (Jack Elston — TEAM FEEDBACK):** Murphy's Pond competes with INSTAAR x2 and Ottawa demo for scheduling priority; execution order is "whatever actually happens first" among competing projects
- **Current focus:** 
  - **URGENT:** Beck Cotter to confirm Flight #4 funding with Dr. Masri (due 2026-05-20)
  - Executing Flight #3 (due 2026-06-30)
  - Preparing S2 aircraft rental order (2 units, training/travel required but not yet scheduled)
  - Resolving FLIR triggering, camera configuration, and methane payload data rate issues

## Notes & Context
- **Research focus:** Methane emissions from wetlands and bottomland hardwood forests; using remote sensing to upscale site measurements to larger geographic areas
- **Field location:** 7192 State Route 307 North, Fancy Farm, KY (landowner Joe Wilson, cwilson0036@gmail.com)
- **Budget & funding:** Project required competitive bidding due to >$10k threshold; Dr. Masri confirmed July/August 2025 start with vertical takeoff capability. Additional $31k available for potential 4th flight (offered July 2025); confirmation due 2026-05-20.
- **Contact details:** Dr. Bassil El Masri, belmasri@murraystate.edu, (270) 809-3110; Murray State University, 415 Blackburn, Murray, KY 42071
- **Billing:** PaymentWorks
- **Products/Services:** S2 VTOL, Survey Services, Aircraft Rental (S0 VTOL specified for rental), Other
- **Customer type:** Commercial; no subcontractors
- **Priority:** Low
- **Funding constraint:** Funding expires December 31, 2026 — all project work must conclude by this date
- **Technical history:** 
  - S2 experienced software issues during August 1 test flight; reverted to Oklahoma State's known-good configuration (Nate Straus, Aug 13, 2025)
  - FLIR triggering issues unresolved; camera overlap (FLIR + A5100) target 80% pending configuration update by Maciej Stachura
  - Methane payload data rate investigation pending (Jack Elston to investigate)
- **Scheduling context:** Project subject to competing demands with INSTAAR x2 and Ottawa demo projects. Execution order is "whatever actually happens first" (Jack Elston, 2026-04-20).