# [035-1] ADONIS Unmanned Experts

## Overview
- **Client/Customer:** Unmanned Experts Inc. (Contact: Keven Gambold, CEO, 303-717-0031, kagambold@unmannedexperts.com)
- **Dollar Value:** $1,249,945 total project budget; $190,000 funding to Black Swift Technologies
- **Timeline:** September 2024 - May 2026 (21 months); Project Due: February 27, 2026
- **Status:** Active - final stage (90% tasks complete, final report and invoice pending)
- **Team Members:** Alex Lomis (Project Owner/Primary), Jack Elston, Josh Fromm, Maciej Stachura, Nate Straus (PIC), Meredith O'hara Needham, Beck Cotter
- **Risk Signals:** Final report milestone overdue (originally due May 18, 2026); final invoice due April 10, 2026. Project winding down successfully post-flight test completion.

## Key Deliverables & Milestones
1. **Deliverable 1: Engineering Prototype (A1) Hardware Freeze** - Oct-Nov 2024 ✓ (Completed Jan 23, 2025)
2. **Deliverable 2: A1 Flight Trials** - Dec 2024-Mar 2025 ✓ (Completed Apr 10, 2025)
3. **Deliverable 3: A1 Swarming Flight Tests** - Apr-Jun 2025 ✓ (Completed Jul 9, 2025)
4. **Deliverable 4: A2 Interim Flight Tests** - Jul-Oct 2025 ✓ (Completed Nov 19, 2025)
5. **Deliverable 5: Mothership-dropped A2 Flight Tests** - Nov 2025-Feb 2026 ✓ (Completed Mar 18, 2026)
6. **Deliverable 6: Final Report** - Mar-May 2026 (In Progress; due Apr 1, 2026)

**Aircraft Deliverables:**
- 2x S0 VTOL ✓ (delivered Jan 22, 2026)
- 4x S0 AD ✓ (delivered Jan 22, 2026)

## Task Summary
- **Total Tasks:** 42 (4 open, 38 completed - 90% completion rate)
- **Tasks by Assignee:**
  - **Alex Lomis:** 28+ completed tasks (primary technical lead) - hardware design, component mounts, aircraft integration, UMEX hardware integration, flight test coordination, milestone completion
  - **Jack Elston:** 4+ completed tasks - autopilot integration, simulation setup, UMEX hardware bench testing
  - **Josh Fromm:** 1 completed task - S0 AD specifications
  - **Meredith O'hara Needham:** 2+ tasks - administrative (deliverable clarification, final invoice pending)
  - **Beck Cotter:** 1 completed task - launch meeting coordination
  - **Unassigned:** 2 open milestones (Final Report)
- **Notable Patterns:** Milestone-driven structure with formal reporting requirements; hardware-heavy integration work completed by Alex Lomis; extensive custom component design and manufacturing; training/travel required for pilot operations (Nate Straus, June 23-26)

## Recent Activity
- **March 18, 2026:** Completed MS7 Swarming S0 AD Flight and MS0007 Mothership-dropped Swarming Flight milestone (5 days after due date)
- **January 22, 2026:** Delivered all aircraft (2x S0 VTOL, 4x S0 AD) and completed MS6 Air Deployed Air Commons Flight test
- **November 19, 2025:** Completed MS4 Swarming Flight (S0 VTOL) and MS0005 Interim A2 Hardware Flight
- **Pending (Apr 2026):** Final Report due Apr 1; Final Invoice due Apr 10

## Notes & Context

**Project Scope:** Three main tasks supporting Unmanned Experts' autonomous sUAS swarming technology:
1. Autonomous sUAS integration - design, iteration, and build of autonomous swarming capability with multiple aircraft
2. Large aircraft integration - design and integration of swarm release mechanism for mothership deployment
3. Transition & Commercialization Preparations - support for licensing, regulatory comprehension, and commercialization planning

**Key Technical Elements:**
- Integration of UMEX Air Commons swarming hardware with BST autopilot systems (FMS, PNT, V2V communication)
- V2V swarming capabilities on S0 platforms
- Air-dropped deployment from mothership aircraft using swarm release mechanism
- Custom component mounts and housing designs for S0 VTOL and S0 AD platforms
- Autopilot simulation setup support for UMEX team (resolved issues Feb 2025)
- Firmware upgrades on ADONIS VTOL hardware (completed Nov 5, 2025)

**Contract Details:**
- Prime Contract: FA8649-24P-0981 (ADONIS)
- Billing Contact: Bill Cossoff (b.cossoff@unmannedexperts.com), 720 South Colorado Blvd, Penthouse North, Denver, CO 80246
- Subcontractor: Black Swift Technologies (BST)
- Priority: Medium
- Customer Type: Commercial

**Aircraft Configuration:** Scope adjusted in Dec 2024 meeting with Alex - final deliverables are 2x S0 VTOL and 4x S0 AD (rather than 3 of each)
- 2x S0 VTOL: Due early March 2026 ✓
- 2x S0 AD: Due early October 2025 ✓
- 2x S0 AD: Due early January 2026 ✓

**Program Performance:** Project successfully transitioned through all major flight test milestones with minimal delays. All hardware deliverables completed on schedule. Final administrative tasks (report writing, invoicing) now pending completion by April 10, 2026.