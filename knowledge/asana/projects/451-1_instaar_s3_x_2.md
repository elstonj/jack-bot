# [451-1] INSTAAR S3 x 2

## Overview
- **Client/Customer**: INSTAAR, CU Boulder (CU Stable Isotope Lab)
- **Dollar Value**: $120,000 total budget to BST
- **Timeline**: S3 delivery due **February 27, 2026**. Operator training scheduled **August 11–13, 2025**. Launcher pallet expected second week of February 2025.
- **Status**: Active — replacing two damaged S2s with insurance-funded S3 units
- **Team Members**: Josh Fromm (delivery owner), Dan Prendergast (operator training), Kevin Rozmiarek (primary & billing contact)
- **Contact**: Kevin Rozmiarek, kevin.rozmiarek@colorado.edu, (512) 573-8279; Address: 4001 Discovery Dr., Boulder, CO 80303
- **Risk Signals**: 
  - **CRITICAL — Operator Training Travel**: August 11–13 operator training travel **not yet scheduled** (Travel Scheduled: N). Less than 4 months away; must be booked immediately.
  - **Near-term Priority Contention**: Jack (April 20, 2026) flags INSTAAR x2 as competing priority alongside Ottawa demo and Murphy's project — actual delivery sequence TBD.

## Key Deliverables & Milestones
- **S3 Delivery**: Due February 27, 2026 — $120,000 (Josh Fromm)
- **Operator Training**: August 11–13, 2025 — 2 units, travel required but **not yet scheduled** (Dan Prendergast)
- **Launcher Shipping**: 3–4 week delivery; pallet arrival at BST estimated second week of February 2025 (confirmed by Kevin Rozmiarek, January 8, 2025)
- **Estimates Approved**: December 11, 2024

## Task Summary
- **Total Tasks**: 3 open, 0 completed (0% completion rate)
- **Tasks by Assignee**:
  - Josh Fromm: 1 task (S3 delivery, due Feb 27, 2026)
  - Dan Prendergast: 1 task (operator training Aug 11–13, High urgency, travel not scheduled)
  - Unassigned: 1 task (Kevin Rozmiarek contact info, High priority)

## Recent Activity
- **April 20, 2026** (Jack): INSTAAR x2 noted as competing near-term delivery priority alongside Ottawa demo and Murphy's project — "whatever actually happens first" — actual sequence TBD.
- January 8, 2025: Kevin Rozmiarek confirmed 3–4 week launcher shipping timeline; pallet delivery expected second week of February 2025.
- December 11, 2024: Estimates approved.
- No tasks completed to date.

## Notes & Context
- Insurance-funded replacement for two damaged S2 units.
- One S3 includes $10k discount (5% insurance deductible applied).
- **URGENT — Training Travel**: Operator training dates (August 11–13, 2025) are ~4 months away. Travel logistics **must be scheduled immediately**. Coordinate with Dan Prendergast and Kevin Rozmiarek to book flights and accommodations.
- Kevin Rozmiarek contact task is high priority — confirm details and file for billing/communication.
- Project Priority: Medium | Customer Type: Commercial