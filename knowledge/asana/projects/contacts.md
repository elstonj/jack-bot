# Contacts

## Overview
- **Client/customer**: Internal contact management system for Black Swift Technologies (BST)
- **Dollar value**: Not applicable (contact database)
- **Timeline**: No specific timeline - ongoing contact management
- **Status**: Active (green status update from April 21)
- **Team members involved**: Beck Cotter (owner), Whole BST Team
- **Risk signals**: None identified

## Key Deliverables & Milestones
This is a contact database, not a project with deliverables. Contains 90 contacts across:
- **Government agencies**: NASA (Ames, Goddard, Langley, JPL), NOAA, USDA, Army, Navy (NAVAIR/NAWCAD), USAF, USGS
- **Universities**: Stanford, UC Boulder, UC Santa Cruz, Oklahoma State, Notre Dame, Embry-Riddle, Murray State, University of Maryland Eastern Shore, University of Miami, Old Dominion, University of Washington
- **Commercial companies**: 
  - Contractors/service providers (manufacturing, consulting, legal, accounting, insurance)
  - Freight/logistics providers
  - Software/technology partners
  - Business development consultants
  - Weather/climate services
  - Aerospace/aviation specialists
  - Communications/ISP providers

## Task Summary
- **Total tasks**: 90 open, 0 completed
- **Tasks by assignee**: All unassigned (contact records only)
- **Data structure**: Each "task" represents a contact with structured custom fields:
  - Company/Agency/Name
  - Email addresses (including personal/alternate contacts where relevant)
  - Physical addresses
  - Phone numbers
  - Websites
  - Priority levels (High, Medium, Low)
  - Product associations (S0, S2, S3, E2, Aeropod, Design/prototyping)
  - Notes with context (roles, upcoming events, special instructions)

## Recent Activity
- Status update from April 21 shows project is active
- **New contact added**: Comcast (Cobb Vaughan, Account Manager) — office: 720-924-3070, cell: 720-670-7191
- Notable upcoming activities & milestones:
  - **May 5, 2025**: Invoice 1666 submitted for S2 parts (Prof. Fu Kuo Chang, Stanford)
  - **July 2025**: Tim Handley (UC Santa Cruz) planning visit to Boulder
  - **August 2025**: Geoffrey Bland (NASA Ames) retiring
  - **September 2025**: FreightPros possibly being replaced by Aeronet as freight shipper
  - **April (annual)**: Atwood Insurance Group contact for health plan renewal

## Notes & Context

### Purpose & Structure
- **Purpose**: Commercial, federal agency, and university contact management for BST operations
- **Integration**: Uses Asana Gmail add-in for email communication (look up contact name as task)
- **Data governance**: Project-specific information stays within individual projects, not with contact records
- **Key note**: If a contact is connected to an active or closed project, they are connected to that project as well

### Recently Added/Updated Contacts (April 2025)
- **Trenton McKibben** (NexTech Solutions LLC) — S3 product, Arctic demo with Canadian Joint Forces Command
- **Luke Peffers** (Nexus Weather & Climate) — Longmont, CO based
- **Mark Bowersox** (Urban Sky) — Denver, CO; stratospheric balloons with NASA
- **Anthony Brescia** (NAVAIR) — NAWCAD Avionics Engineering S&T Domain Lead, S0 & Design/prototyping
- **Angel Ruiz-Reyes** (NAVAIR/NAWCAD) — Physicist, Advanced Technology Development Department; **TPOC for Navy SBIR magnetometer project**; S0 & Design/prototyping
- **Russ Evans** (USAF/AFMC) — Electrical Engineer (PCI Aviation), S0 product
- **Ryan Rickert** (USAF 53rd Weather Reconnaissance Squadron) — Chief Meteorologist, S0 product
- **Elizabeth Sanabia** (University of Washington Applied Physics Lab) — Senior Principal Oceanographer, retired US Navy Captain
- **Cobb Vaughan** (Comcast) — Account Manager, office: 720-924-3070, cell: 720-670-7191

### High-Priority Contacts
- **Sabu Best** (Barbados Meteorological Services)
- **Geoffrey Bland** (NASA Ames, retiring Aug 2025) — Aeropod product
- **Bassil El Masri** (Murray State University)
- **Christopher Hartman** (University of Maryland Eastern Shore) — S3 product
- **Kevin Rozmiarek** (CU Stable Isotope Lab)
- **Angel Ruiz-Reyes** (NAVAIR) — Navy SBIR magnetometer project TPOC

### Product Associations
- **S0**: Tanay Topac (Stanford), Kevin Adkins (Embry-Riddle), Anthony Brescia (NAVAIR), Angel Ruiz-Reyes (NAVAIR), Russ Evans (USAF), Ryan Rickert (USAF)
- **S2**: Prof. Fu Kuo Chang (Stanford), Tanay Topac (Stanford)
- **S3**: Christopher Hartman (University of Maryland Eastern Shore), Trenton McKibben (NexTech Solutions LLC)
- **E2**: Daniel Hesselius (Univ. of Colorado)
- **Aeropod**: Geoffrey Bland (NASA Ames)
- **Design/prototyping**: Anthony Brescia (NAVAIR), Angel Ruiz-Reyes (NAVAIR)

### Service Providers & Key Vendors
- **Communications/ISP**: 
  - Comcast (Cobb Vaughan, Account Manager) — 720-924-3070
- **Freight/Shipping**: 
  - Aeronet Worldwide (new primary shipper, replacing FreightPros) — Patrick Mulstay contact
  - WorldWide Express/WWEX (UPS authorized reseller as of 2025) — Kelsie Pierson account manager
  - FreightPros (legacy, being phased out Sept 2025)
- **Insurance**: 
  - Atwood Insurance Group (health) — Chad Atwood
  - Bolder Insurance (aviation) — Debbie Rampy
- **Legal**: Koenig, Oelsner, Taylor, Schoenfeld & Gaddis PC (transactional law) — Dana Fredrickson contact
- **Manufacturing**: 
  - MM Solutions (custom UAS cases) — Foster Hill
  - Northwind Composites (UAV manufacturing) — Lee Skidmore
- **Accounting/Tax**: 
  - JB Advisory Group (CPA) — Julie Beth Malone
  - Jones & Associates CPAs PC — Bill Jones
  - Boostr Consulting LLC (R&D tax credits) — Thierry Lepoutre
  - GovIRG LLC (gov't contracting & accounting) — Kevin Hoskins
- **Consulting**: 
  - Boreas Consulting (management) — Gijs de Boer
  - P3 Tech Consulting (UAS issues) — Dawn Zoldi
  - ByLight Professional IT Services (design/prototyping) — Bryan Sparling, Tech Lead Jeff Adelman
- **Weather/Climate Services**:
  - Nexus Weather & Climate — Luke Peffers

### Government/Invoice Portals
- **PIEE**: Navy/Air Force invoice submission portal
- **IPP**: US Treasury portal for NASA/NOAA invoices

### Notable Relationships & Context
- **University Subcontracting Chain**: BST (Prime) → ERAU (sub) → University of Miami (Jun Zhang)
- **Flight Testing**: Sunny Slope Sod Farm LLC (Longmont, CO)
- **Pilot Resources**: 
  - Rocky Mountain High Aviation (D. Hesselius, drone pilot)
  - DSKinetic LLC (Spencer Lisenby, subcontractor pilot)
- **Aerospace Association**: Boulder Aeromodeling Society (Nate and Jack are 2025 members)
- **Landlord**: W. W. Reynolds Companies (Wilderness Pl location) — Amy Smith
-