# [550-2] Navy STTR: Hazardous Weather

## Overview
- **Client/Customer**: Department of the Navy (Office of Naval Research); TPOC: Joshua Cosseth (joshua.h.cossuth.civ@us.navy.mil, 407-696-0703)
- **Dollar Value**: $146,326 total budget ($138,494 to BST, $66,290 to subcontractors)
- **Timeline**: 
  - Base Period: July 7, 2025 – January 6, 2026 ✓ **COMPLETED**
  - Option Period: January 7, 2026 – July 6, 2026 ✓ **COMPLETED**
  - Final deliverables due: **September 1, 2026**
- **Status**: **Active — Option Period work completed; Final Report & Invoice pending. One open task (Pioneer Array Flight) with July 28, 2026 due date.**
- **Team Members**: 
  - **Maciej Stachura** (project owner; primary technical lead)
  - **Alex Lomis** (Pioneer Array Flight testing)
  - **Alex & Sam** (critical engineering work on mag integration)
  - **Meredith O'hara Needham** (invoicing and administration)
  - **Jack Elston** (supporting deliverables)
  - **Beck Cotter** (contract management)
  - **Daniel Prendergast** (EMASS validation flights)
  - **BST Whole Team**
- **Subcontractors**: 
  - **ERAU** / Josh Wadler (wadlerj@erau.edu; 1 Aeronautical Blvd, Daytona Beach, FL 32114; Ph: 386-226-6000); Pre-award contracting officer: Garrett Filson (filsong@erau.edu)
  - **Old Dominion University** / John Park (h1park@odu.edu; 2101F Engineering Systems Bldg, Norfolk, VA 23352; Ph: 757-683-3759); Pre-award contracting officer: Stephanie Haskell (sl2harri@odu.edu)
  - **University of Miami** / Jun Zhang (jun.zhang@earth.miami.edu; Ph: 305-979-0903); **CRITICAL: must be included on all correspondence**; funds flow: BST (Prime) → ERAU (Sub) → U Miami (ERAU Sub)
- **Risk Signals**: Pioneer Array Flight due July 28, 2026 (approaching); Final Report (CLIN 0007) and Final Invoice (CLIN 1007) due September 1, 2026.

## Key Deliverables & Milestones
| Deliverable | Due Date | Value | Status |
|---|---|---|---|
| Base Period Kickoff | July 22, 2025 | $70,000 | ✓ Completed |
| Base Period Progress Report | October 6, 2025 | $49,000 | ✓ Completed |
| Base Period Final Report | January 5, 2026 | $20,826 | ✓ Completed |
| TABA Report | January 5, 2026 | $6,500 | ✓ Completed |
| Option Period Kickoff | April 2, 2026 | $50,000 | ✓ Completed |
| Phase II Cal/Val Plan | May 18, 2026 | — | ✓ Completed June 1, 2026 |
| Option Progress Report (CLIN 0006) | June 3, 2026 | $35,000 | ✓ Completed June 4, 2026 |
| Option Invoice (CLIN 1006) | June 3, 2026 | $35,000 | ✓ Completed June 25, 2026 (22 days late) |
| **Pioneer Array Flight** | **July 28, 2026** | — | **OPEN — Assigned to Alex Lomis** |
| **Option Final Report (CLIN 0007)** | **September 1, 2026** | **$14,446** | **OPEN — In Progress** |
| **Option Final Invoice (CLIN 1007)** | **September 1, 2026** | **$14,446** | **OPEN — Ready to Submit** |

## Task Summary
- **Total Tasks**: 1 open, 0 completed
  - **Open Tasks** (1):
    - Pioneer Array Flight | Alex Lomis | Due: July 28, 2026
- **Critical Action Items Pending**: 
  1. Pioneer Array Flight (test/validation) | Alex Lomis | Due: July 28, 2026
  2. OPTION CLIN0007: Final Report | Maciej Stachura | Due: September 1, 2026
  3. Submit CLIN 1007 Invoice ($14,446) | Meredith O'hara Needham | Due: September 1, 2026

## Recent Activity
- **July 20, 2026** (Historical): ✓ **ISARRA data task completed** by Maciej Stachura (45 days past original June 5 due date; work was proceeding on critical mag integration engineering).
- **June 25, 2026**: ✓ **CLIN 1006 Invoice ($35,000) successfully submitted** via WAWF (22 days late, but completed).
- **June 8, 2026**: Strategic discussion with Navy TPOC — Navy expressed interest in **focusing on commercial markets and expanding non-DOD weather applications**. Aligns with Phase II planning; no contract changes indicated.

## Notes & Context
- **Project Purpose**: Refining the S0 platform and demonstrating its utility for operational Navy use through targeted research and development.
- **Invoicing**: All payments processed through Wide Area Workflow (WAWF). Meredith O'hara Needham manages BST invoicing submissions. Final Invoice (CLIN 1007; $14,446) due September 1, 2026.
- **Subcontractor Coordination**: Jun Zhang (University of Miami) **must be included on all correspondence**, though funds flow through Josh Wadler at ERAU. Pre-award contracting officers: Garrett Filson (ERAU, filsong@erau.edu); Stephanie Haskell (ODU, sl2harri@odu.edu).
- **Commercial Market Strategy**: Navy TPOC indicated interest in BST's industrial plan for commercial market expansion and non-DOD weather applications; aligned with Phase II strategic planning.
- **Next Steps**: 
  1. Complete Pioneer Array Flight testing (Alex Lomis) by July 28, 2026.
  2. Finalize Option Final Report (CLIN 0007) for September 1, 2026 deadline (Maciej Stachura).
  3. Prepare and submit Final Invoice (CLIN 1007; $14,446) by September 1, 2026 (Meredith O'hara Needham).