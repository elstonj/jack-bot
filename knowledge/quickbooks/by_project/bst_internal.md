# BST Internal — QuickBooks Financials

## Financial Summary
- **Total Invoiced (Revenue)**: $17,894.36
- **Total Expenses (Bills + Purchases)**: $1,099,337.82 *(+$940.52 from June 5–18, 2026)*
- **Total Purchase Orders**: $1,700.00 *(PO #1038 Amprius, outstanding)*
- **Net Position**: -$1,081,443.46 (expenses exceed revenue significantly)
- **Date Range**: April 5, 2024 – June 18, 2026 (26+ months)
- **Transaction Count**: 1,186 *(all confirmed)*

---

## Revenue (Invoices & Payments)

| Invoice # | Date | Customer | Amount | Balance |
|-----------|------|----------|--------|---------|
| #1752 | 2026-04-03 | Krateo Sky | $10,860.48 | $10,860.48 |
| #1727 | 2025-12-02 | Weather Stream (c) | $928.88 | $928.88 |
| #1696 | 2025-08-26 | Phase1 Aviation | $6,105.00 | $6,105.00 |

**Total Invoiced**: $17,894.36  
**Total Collected**: $0.00 (all invoices remain outstanding)  
**Outstanding Receivables**: $17,894.36

**Notable**: Invoice #1696 includes accidental double-payment to Phase1 Aviation for 20 wings ($6,105.00). This amount should be recovered or credited.

---

## Expenses by Cost Category

### Direct Labor & Subcontractors: $389,501.30
**Total: $389,501.30** | **24 vendors** | **Largest: Matthew Crabtree $108,181.68**

No new transactions in June 5–18 period in this category.

---

### Materials & Direct Purchases: $520,989.60
**Total: $520,989.60** | **72 transactions** | **Largest individual items: S3 inventory $33,962.84, Electronics/components $209,099.64, Composites/structures $46,854.95**

No new transactions in June 5–18 period in this category.

---

### IRAD Indirect R&D (Subcontractors & Internal): $81,753.68
**Total: $81,753.68** | **68 transactions** | **Largest: Boostr R&D Tax Credits $10,773.62; Matthew Crabtree R&D Assembly $4,954.00; MicroFirm Engineering $6,257.60**

**Previously Confirmed Transactions (June 2–4, 2026):**
- Purchase #SW46G546: $809.58 — S3 IRAD purchase
- Purchase #69060: $1,926.40 — Nosecone ring x 3
- Purchase #0529JELSTON: $78.05 — Neoprene Vibration-Damping Sandwich Mount
- Purchase #69038: $140.59 (June 11) — S3 IRAD shipment to Sean Cole
- Purchase #260604: $235.98 (June 4) — ARK 4IN1 ESC (Electronic Speed Controller)

**New Confirmed Transactions (June 5–11, 2026):**

**Purchase #0604JELSTON**: $165.46 (June 5, 2026)
- **Account**: [RESEARCH & DEVELOPMENT:IRAD-Internal Research and Development]
- **Description**: *(Unspecified component)*
- **Memo**: No Asana request - project listed under item purchased
- **Status**: ✓ Confirmed
- **Impact**: S3 R&D component; part of ongoing prototyping/development cycle
- **Pattern**: Consistent with multi-system S3 development initiatives

**Purchase #69119**: $207.31 (June 5, 2026)
- **Account**: [RESEARCH & DEVELOPMENT:IRAD-Internal Research and Development]
- **Description**: Part-29-Main-1-.step (structural/mechanical component)
- **Status**: ✓ Confirmed
- **Impact**: S3 structural/airframe development; likely composite or metal airframe component
- **Pattern**: Aligns with concurrent nosecone rings (June 2–3) and ESC procurement (June 4); multi-subsystem integration phase

**Purchase #69038**: $140.59 (June 11, 2026)
- **Account**: [RESEARCH & DEVELOPMENT:IRAD-Internal Research and Development]
- **Description**: S3 IRAD shipment to Sean Cole
- **Status**: ✓ Confirmed
- **Impact**: Internal R&D distribution; Sean Cole assembly/integration work
- **Pattern**: Fulfills component set for downstream assembly

**Status**: June 5–18 period shows **$513.36** in new IRAD R&D component purchases, continuing multi-subsystem S3 development (airframe + flight systems + integration).

---

### Sales & Marketing: $5,142.11
**Total: $5,142.11** | **3 transactions**

**Previously Confirmed Transactions (June 2, 2026):**
- Purchase #1315-5343: $5,019.30 — Yearly subscription

**Previously Confirmed Transactions (June 4, 2026):**
- Purchase #PIRATE SHIP * POSTAGE - 7333: $55.44 — Lee Skidmore shipment to BTS for S3 Sales

**New Confirmed Transactions (June 5, 2026):**

**Purchase**: $66.37 (June 5, 2026)
- **Account**: [SALES & MARKETING]
- **Description**: Labels for wine (Krateo Sky visit)
- **Memo**: No Asana request - Paige made purchase directly
- **Status**: ✓ Confirmed
- **Impact**: Direct customer engagement; Krateo Sky visit support material (branded merchandise/hospitality)
- **Pattern**: Tactical sales execution; customer relationship building
- **Significance**: **SECOND documented customer engagement in 48 hours** (BTS June 4, Krateo Sky June 5)
- **Timing**: Aligns with elevated marketing spend ($5,019.30 annual subscription, June 2) and aggressive sales pipeline activation

**Status**: June 5–18 period shows **$66.37** in new S&M expenses, reflecting active customer engagement (Krateo Sky visit support).

---

### Inventory Asset (S3 Sales & S2 Production Scaling): $4,414.68
**Total: $4,414.68** | **17 transactions** | **Largest: S3 sales components (June 18)**

**Previously Confirmed Transactions (June 3–4, 2026):**
- Purchase #1083722: $907.80 (June 3) — E+E EE03 OEM Humidity/Temperature Module x4
- Purchase: $48.96 (June 4) — Shipping for 123 Bearing Shipment

**Confirmed Transactions (June 11–18, 2026):**

**Purchase #SC51C906**: $158.58 (June 18, 2026)
- **Account**: [Inventory Asset]
- **Description**: For future S3 sales
- **Status**: ✓ Confirmed
- **Impact**: Component procurement for S3 production scaling; inventory replenishment
- **Pattern**: Aligns with June 3–4 bearing modules and bearing shipment; continued production ramp

**Purchase #YW1744139**: $302.21 (June 18, 2026)
- **Account**: [Inventory Asset]
- **Description**: For future S3 sales
- **Status**: ✓ Confirmed
- **Impact**: Component procurement for S3 production scaling; inventory replenishment
- **Pattern**: Sequential same-day purchase (with #SC51C906); indicates coordinated multi-component procurement cycle

**Status**: June 5–18 period reflects **$460.79 cumulative inventory asset purchases** (both transactions dated June 18), continuing S3 production scaling initiatives. Total inventory asset spending in June 2026: **$1,418.55** across 4 transactions.

---

### Rent & Facilities: $52,923.63
**Total: $52,923.63** | **5 transactions** | **Largest: W. W. Reynolds Companies $7,059.94**

No new transactions in June 5–18 period in this category.

---

### General & Administrative (Legal, Tariffs, Utilities, Accounting): $46,520.64
**Total: $46,520.64** | **4 transactions** | **Largest: KO Law PC Investment Transaction $44,964.50; Tariff/Import Duties $498.57; Accounting Fees $360.00**

No additional new transactions in June 5–18 period beyond June 2 accounting services ($360.00).

---

### Fringe Benefits (Retirement Contributions): $534.28
**Total: $534.28** | **1 transaction**

No new transactions in June 5–18 period in this category.

---

## Purchase Orders
| PO # | Date | Vendor | Amount | Description | Status |
|------|------|--------|--------|-------------|--------|
| #1038 | TBD | Amprius | $1,700.00 | Battery/power component | Outstanding |

---

## Monthly Spend by Category

### June 2026 (Updated: June 5–18) — Revised Summary
| Category | Amount | # Transactions | Trend |
|----------|--------|-----------------|-------|
| IRAD Indirect R&D | $513.36 | 3 | → S3 airframe + integration components (June 5, 11) |
| Sales & Marketing | $66.37 | 1 | ↑ Krateo Sky customer engagement (June 5) |
| Inventory Asset (S3 Production) | $460.79 | 2 | ↑ S3 component procurement (June 18) |
| **June 5–18 Subtotal** | **$1,040.52** | **6** | Customer engagement + continued R&D + inventory prep |

### Cumulative June 2026 (June 1–18) — Full Month Summary
| Category | Amount | # Transactions |
|----------|--------|-----------------|
| Sales & Marketing | $5,142.11 | 3 |
| IRAD Indirect R&D | $3,622.93 | 8 |
| Inventory Asset | $1,418.55 | 4 |
| General & Administrative | $360.00 | 1 |
| **June YTD Total** | **$10,543.59** | **16** |

### Key Patterns (June 2–18)
- **S3 Multi-Phase Acceleration**: $10,543.59 in 18 days reveals coordinated push across three vectors:
  1. **R&D Flight Systems & Airframe** ($3,622.93): ESC, nosecone rings, environmental sensors, airframe components (Part-29-Main-1-.step), assembly/integration
  2. **Production Inventory** ($1,418.55): Humidity/temperature modules (qty 4), bearings, S3 sales components (June 18 procurement cycle)
  3. **Sales Execution** ($5,142.11): Marketing platform ($5,019.30) + TWO customer engagements ($55.44 BTS + $66.37 Krateo Sky)
  
- **Dual Customer Engagement Signal (48-hour window)**:
  - **June 4**: Lee Skidmore shipment to BTS — first documented sample shipment
  - **June 5**: Krateo Sky visit support (branded wine labels) — direct customer visit/hospitality
  - **Implication**: Two parallel customer evaluation tracks (BTS + Krateo Sky) activated simultaneously within days of $5K annual marketing subscription commitment
  
- **Component Procurement Pattern (June 5–11)**:
  - Environmental sensing (E+E EE03 humidity/temp modules, June 3)
  - Flight control (ARK 4IN1 ESC, June 4)
  - Airframe/Structural (Part-29-Main-1-.step, June 5