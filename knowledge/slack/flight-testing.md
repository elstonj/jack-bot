...
**Key Participants:** Maciej (lead flight testing/analysis), Jack Elston (firmware development), Joshua Fromm (QA/component sourcing), Ben Busby (tablet/GCS software), Nate (videography/pilot), Alex Lomis (field operations/media/pilot), Sam Hild (Remote ID integration/test pilot/magnetometer troubleshooting), Dan Prendergast (autopilot/control systems/operations), Danny Troke (data management), Ethan Domagala (pilot), Beck Cotter (operations support), Paige Smith (media/video curation), Meredith Needham (logistics/administrative support), Scott (equipment/field operations), Stefan (payload data processing - CH4/CO2), Spencer Hoehl (QA/maintenance workflow coordination), Sunny Slope Sod Farm (external airspace contact/CU training partner), Chris & Kevin (external clients/stakeholders), Christoph & Angie (internal stakeholders), Dan H. (external contact/AUVSI show), Ken Jochim (BAS airfield operator), Boulder Emergency Squad (external visitor/stakeholder), NRL (external client/payload testing)

**Activity Level:** Extremely high — 5,600+ messages across June 2020 through July 2026. Daily operations with multiple aircraft flights, rapid issue turnaround, continuous firmware iterations, and intensive VTOL transition testing. Current activity (July 2026) shows ongoing S3 and S1-VTOL deployment with methane (CH4) and CO2 sensing payload data processing, AGL laser altitude data integration, magnetometer analysis, and quaternion (attitude) logging enhancements for S1-VTOL failure testing validation and flight control analysis. Increased focus on Navy magnetometer testing and external stakeholder engagement. Latest activity (July 24, 2026) indicates NRL (Naval Research Laboratory) gimbal payload testing planning for upcoming week flights, with discussions around mass model integration for flight stability and control.
...
- **May 14, 2026:** S1-VTOL failure testing comprehensive test plan approved for May 15 flight operations; includes mag failure landing (gyro integration), loss of laser on landing, low battery flight terminate with transition, joystick recovery modes, and