...
- **Scheduling is systemd user timers, not cron** (`scripts/systemd/`, installed to `~/.config/systemd/user/`). `jackbot-scan.timer` fires 02:03 daily, `jackbot-digest.timer` 07:27 weekdays. Both set `Persistent=true`, which is the whole point: the laptop is asleep at 02:03 and **cron silently skips missed runs**, so the scan stopped after 2026-08-04 and went unnoticed for three weeks while briefings and Q&A kept answering confidently from stale files. `Persistent=true` turns a missed window into a late run on wake. `Linger=no`, so timers run only while a login session is active — fine for a daily-driver laptop; neither timer can wake a sleeping machine. Operate with `systemctl --user list-timers 'jackbot-*'` and `journalctl --user -u jackbot-scan.service`
- **A missing scan can't report itself**, so `scan_freshness.py` ages the newest timestamp in `knowledge/.scan_state.json` and warns past 48h. Checked in two places: `scripts/daily_digest.py` (local, prepended above the bug list) and `daily_research.py` (posts an `[ERROR]`) — the latter runs on Railway, which is always up, so the alarm fires even if the laptop never wakes. A missing or corrupt state file warns rather than reading as fresh. Coverage: `python test_scan_freshness.py`
