...
- **Morning digest** — `scheduler.py::post_commercial_sales_digest` runs at 8:02am Mon-Fri, loads JSONs only (no LLM at post-time). Posts a single header message to `#commercial-sales` ("Customer Builds & Support — <date>"), then posts every Build / SupportCase card as a **threaded reply under that umbrella header** so the channel main view stays clean. Cards still carry owner pings, state, contents, parts checklist, and missing-info callouts routing each gap to its responsible owner (Beck/Meredith/Nate per `BUILD_FIELD_ROLES` in `commercial_sales.py`). The scheduler writes `_message_map.json` with `umbrella_ts`, an ordered `cards: [{ts, kind, id, customer, label}]` list, and a flat `messages: {ts → {kind, id}}` map (used for both per-card direct lookup and content-match routing in the reply handler).
